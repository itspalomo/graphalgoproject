...
void Graph::Dijkstra_fh(int s) {
    initialize_source(s);                         // set all but s's depth to 'infinity'

    FibHeap<Vertex*> H;                             // Q <- G.V
    H.insert(&G[s]);

    while (!H.empty()) {

        Vertex* u;
        u = H.min();                    // u = Extract-Min()
        H.Extract_min();
